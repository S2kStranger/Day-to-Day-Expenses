...
btnincome.addEventListener('click',(e) => 
{
  ...
  else
  {
    ...
    axios.post("http://3.82.37.119:4000/updateIncome",obj,{headers:{"Authorization":token}})
      .then(() => {
        console.log("success");
...
window.addEventListener('DOMContentLoaded',async (e) => {
  ...
  try
  {
    ...
    const result = await axios.get("http://3.82.37.119:4000/account/getexpenses",{headers:{"Authorization":token}});
...
async function fetchDownloadLinks(currentPage)
{
    ...
    const token = localStorage.getItem("token");
    const result_link = await axios.get(`http://3.82.37.119:4000/account/pagination/${rowsPP}/${currentPage}`,{headers:{"Authorization":token}});
...
async function premiumFeature()
{
  ...
  lb.style.visibility='visible';
    const lbdata = await axios.get("http://3.82.37.119:4000/account/premium/getfeature");
...
function addInTable(obj) {
  ...
  delbtn.addEventListener('click',(e) => {
    ...
    const token = localStorage.getItem('token');
    axios.delete(`http://3.82.37.119:4000/account/deleteExpense/${expenseid}`,{headers:{"Authorization":token}})
      .then(result => {
        tr.remove();
        location.reload();
      })
...
{
  const token = localStorage.getItem('token');
  axios.get("http://3.82.37.119:4000/account/download",{headers:{"Authorization":token}})
    .then(response => {
...
myform.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const token = localStorage.getItem('token');
    const newData = await axios.post("http://3.82.37.119:4000/postnewexpense",obj_data,{headers:{"Authorization":token}});
...
{
  const token = localStorage.getItem('token');
  const response = await axios.get("http://3.82.37.119:4000/purchase/premium_membership",{headers:{"Authorization":token}});
  ...
  var options = 
  {
    ...
    "handler" : async function(response) {
      await axios.post('http://3.82.37.119:4000/purchase/updatetransactionstatus',{
  ...
  rzp1.on('payment.failed',function(response) {
    axios.post('http://3.82.37.119:4000/purchase/failedtransaction',{
      order_id: options.order_id})
    .then(() => {
